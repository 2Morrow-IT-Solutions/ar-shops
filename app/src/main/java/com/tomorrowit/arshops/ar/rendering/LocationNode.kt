...
class LocationNode(
    ...
    override fun onUpdate(frameTime: FrameTime) {
        ...
        for (n in children) {
            ...
            val distanceInAR = sqrt((dx * dx + dy * dy + dz * dz).toDouble())
            ...
            if (locationScene.shouldRemoveOverlapping()) {
                ...
                val xDelta = (distanceInAR * Math.sin(Math.PI / 15)).toFloat() //12 degrees
    ...
    fun scaleAndRotate() {
        for (n in children) {
            ...
            if (scalingMode == LocationMarker.ScalingMode.FIXED_SIZE_ON_SCREEN) {
                scale =
                    sqrt((direction.x * direction.x + direction.y * direction.y + direction.z * direction.z).toDouble())
                        .toFloat()
            } else if (scalingMode == LocationMarker.ScalingMode.GRADUAL_TO_MAX_RENDER_DISTANCE) {
            ...
            } else if (scalingMode == LocationMarker.ScalingMode.GRADUAL_FIXED_SIZE) {
                scale =
                    sqrt((direction.x * direction.x + direction.y * direction.y + direction.z * direction.z).toDouble())
                        .toFloat()
                ...
                scale *= max(gradualScale, gradualScalingMinScale)
            }
            scale *= scaleModifier
